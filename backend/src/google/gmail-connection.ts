...
export async function getGmailOauthLink(environment: Environment, identifier: string, redirectAfterAuth: string) {
    ...
    while (true) {
        ...
        const result = await redis.set(`gmail-state-token:${stateToken}`, JSON.stringify({
...
export async function handleGmailCallback(request: FastifyRequest, response: FastifyReply) {
    ...
    const stateToken: StoredStateToken | null = await redis.get(`gmail-state-token:${state}`).then((data) => data ? JSON.parse(data) : null);
    if (!stateToken) {
...
export async function getGmailMessages(identifier: string, environmentId: string, limit: number = 10) {
    ...
    const messages = await Promise.all(messageIds.map((message) => {
...
const getBody = (part: gmail_v1.Schema$MessagePart): { contentType: "text" | "html", content: string } | { contentType: "text" | "html", content: string }[] | null => {
    if (part.mimeType === "text/plain") {
        ...
        return part.parts?.map(getBody).flat().filter((part) => part !== null) ?? [];
    }
...
function gmailToGeneric(gmailMsg: gmail_v1.Schema$Message, identifier: string, environmentId: string): EmailMessage {
    const payload = {
    ...
    const to = getHeaderValue(headers, "To")?.split(",").map(strToEmailAddress) ?? [];
    const cc = getHeaderValue(headers, "Cc")?.split(",").map(strToEmailAddress) ?? [];
    const replyTo = getHeaderValue(headers, "Reply-To")?.split(",").map(strToEmailAddress) ?? [];
    const date = getHeaderValue(headers, "Date") ?? undefined;
    const references = getHeaderValue(headers, "References")?.split(" ") ?? undefined;
