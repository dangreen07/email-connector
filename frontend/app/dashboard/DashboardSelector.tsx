...
export default function DashboardSelector({
  ...
  const { activeProjectId, activeEnvironmentId } = useMemo(() => {
    ...
    return {
      activeProjectId: projectId ?? projects.at(0)?.id,
      activeEnvironmentId:
        environmentId ?? projects.at(0)?.environments.at(0)?.id,
    };
  }, [pathname, projects]);

  const activeProject = useMemo(
    () => projects.find((p) => p.id === activeProjectId) ?? projects.at(0),
    [projects, activeProjectId]
  );
  const hasProduction = !!activeProject?.environments.some(
    (e) => e.name === "production"
  );
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    onClick={() => {
                      ...
                      startTransition(async () => {
                        ...
                        if (result && "error" in result) {
                          ...
                          router.push(
                            `/dashboard/${result.projectId}/${result.environmentId}`
                          );
                        }
                      });
                    }}
          ...
          <div className="flex items-center justify-between">
            <Tabs
              ...
              onValueChange={(val) => {
                if (activeProject)
                  navigateTo(
                    activeProject.id,
                    val as "development" | "production"
                  );
              }}
            >
              <TabsList>
                {activeProject?.environments?.some(
            ...
            {!hasProduction && activeProject && (
              <Dialog
                open={isCreateProdOpen}
                onOpenChange={setIsCreateProdOpen}
              >
                <DialogTrigger asChild>
